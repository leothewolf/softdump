...
if ($_SERVER['REQUEST_METHOD'] == "POST"){ //check for request
    ...
    mysqli_stmt_close($stmt);//closing stmt 
...
elseif(strlen(trim($_POST['password'])) < 6){//checking if password is less then 6 characters
...
{
    $sql = "INSERT INTO users (username, password) VALUES (?, ?)";//creating sql query
    $stmt = mysqli_prepare($conn, $sql);//preparing statment
    if ($stmt)
    {
        mysqli_stmt_bind_param($stmt, "ss", $param_username, $param_password);//binding parameteres ? ? to $param_username and $param_password
